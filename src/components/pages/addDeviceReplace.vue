...
<div>
    <div class="main_wrap" v-show="mainPageShow">
        <div class="content_wrap" :disabled='isDelay'>
            <div class="device_info">
                <p>更换人<span>{{user.name}}</span></p>
                <p>更换时间<span>{{'' | getTime}}</span></p>
                <p>设备名称<span>{{deviceInfo.name}}</span></p>
...
          }
      ...
      getDeviceInfo: function(){
        if(this.$route.query.deviceId){
          ...
          this.$http(this.$API.getDeviceReplaceRecordByDeviceId, {deviceId:id}, true).then((res) => {
            if(res){
              this.deviceInfo ={
                name:res.deviceName,
                code:res.code,
      ...
      showDate() {
        let that = this
        this.replaceConfig = this.$route.query.device;
        //获取当前的日期
        let date = new Date()
        ...
        this.$http(this.$API.delayedDeviceReplaceDate,idData,true).then(res=>{
          var res = '2019-09-02'
          if(res){
              nowDate = res;
              this.$vux.datetime.show({
                ...
                onConfirm (value) {
                  ...
                  this.isDelay = true
                  if(DateDiff(nowDate,that.delayTime) < parseInt(that.replaceConfig.left_date)) {
              ...
              this.$vux.datetime.show({
                ...
                onConfirm (value) {
                  that.delayTime = value;
                  this.isDelay = true
                  if(DateDiff(nowDaterror,that.delayTime) < parseInt(that.replaceConfig.left_date)) {
                    that.$commom.alert(`选择要更换的时间为${that.delayTime},将会提前更换`);
                  }
                },
                onHide(type) {
