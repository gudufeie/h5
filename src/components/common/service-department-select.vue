          ...
          <div class="listBox" v-for="(levelItem,index) in levelList" :key="index"
            ...
            <div class="list">
              <div class="item" v-for="(item,index) in levelItem.departmentList" :key="index"
                   @click="selectDepartment(item,levelItem.level,index)">
  ...
  export default {
    ...
    methods: {
      buildDepartmentTree: function (departmentList) {
        ...
        if (this.rootDepartment.departmentList.length > 1) {
          this.levelList[0].departmentList.unshift({departmentName: '全企业', id: '1'})
        }
      },
      ...
      selectDepartment: function (department, level,index) {
        if(level == 0 && !!!index){
            this.historySelectedId.first = 0;
        }else if(level == 0 && index){
            this.historySelectedId.first = index;
        }else if(level == 1 && index){
            this.historySelectedId.second = index;
        }else if(level == 2 && index){
